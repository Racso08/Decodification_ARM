ROM		EQU		0x10000 ; Dirección inicial de la ROM
RAM		EQU		0x10420 ; Dirección inicial de la RAM
FFD		EQU		0x10C20 ; Dirección del Flip Flop con la señal de inicio
		
		;		Rutina que se encarga de verificar si se puede iniciar el proceso de decodificación
		
Inicio	LDR		R0, =FFD ; Carga dirección de memoria del Flip Flop en R0
...
Fin		LDR		R0, =FFD ; Carga dirección de memoria del Flip Flop en R0
...
TipoDeco	LDR		R0, =ROM ; Carga dirección de memoria del ROM en R0
...
DecoXOR	LDR		R1, =RAM ; Carga dirección de memoria del RAM en R1
...
DecoNOT	LDR		R1, =RAM ; Carga dirección de memoria del RAM en R1
		MOV		R2, #4   ; Almacena un 4 en R2
		MOV		R3, #0   ; Almacena un 0 en R3
...
DecoROR	LDR		R1, =RAM ; Carga dirección de memoria del RAM en R1
		MOV		R2, #4   ; Almacena un 4 en R2
		MOV		R3, #0   ; Almacena un 0 en R3
